...
```bash
docker plugin install --alias cloud1 ghcr.io/mkapusnik/glusterfs-volume GFS_VOLUME=cloud1 GFS_SERVERS=server1,server2,server3
```
...
```bash
make build push plugin=docker.lan/mkapusnik/gluster-volume
```
